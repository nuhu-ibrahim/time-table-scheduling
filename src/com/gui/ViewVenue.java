...
public class ViewVenue extends javax.swing.JInternalFrame {
    ...
    private void createTable(){
        String sql="select class_room_name, space_capacity from class_room";

        JTable table;
        TableColumn column = null;
        JScrollPane scrollPane;
        String meta[]={"Venue Name", "Space Capacity"};
        table= MyTable.createTable(sql, meta);
        table.setPreferredScrollableViewportSize(new Dimension(480, 200));
        table.setToolTipText("Select venue record and click edit or delete to edit or delete information");
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                try{
                    ...
                    class_room_name = table.getModel().getValueAt(row, 0).toString();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("EDIT/DELETE VENUE INFORMATION");
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        try{
            int confirm = JOptionPane.showConfirmDialog(this, "Delecte Venue Information? Notice that all information related to the venue will also be deleted.", "Confirmation", JOptionPane.YES_NO_OPTION);
            PreparedStatement st;
            
            if(confirm == JOptionPane.YES_OPTION){
                String sql = "delete from class_room where class_room_name = ?";
                st = DBConnect.con.prepareStatement(sql);

                st.setString(1, class_room_name);
                
                st.executeUpdate();
                
                JOptionPane.showMessageDialog(this, "Venue information has been deleted","Success",JOptionPane.INFORMATION_MESSAGE);
    ...
    private void editActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editActionPerformed
        try{
            ...
            int class_room_id = DBConnect.rs.getInt("class_room_id");

            this.dispose();
            this.hide();
            
            EditVenue edit_venue = new EditVenue(class_room_id);
            desktop1.add(edit_venue, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0));
            edit_venue.setSelected(true);
        }catch (Exception e) {
            System.err.println(e.getMessage());
            e.printStackTrace();
        }
